...
import { Check, ChevronDown, FileCheck, FileSearch, FileX, FileX2Icon, Search, X } from "lucide-react";
...
export default function Creategroup() {
  ...
  const handleMemberClick = (user) => {
    if (user.username === currentUser) return; // Prevent adding current user
    setGroupData((prev) => ({
      ...prev,
  ...
  const handleAdminClick = (user) => {
    ...
    setGroupData((prev) => ({
      ...prev,
      admins: prev.admins.includes(user.id)
        ? prev.admins.filter((id) => id !== user.id)
        : [...prev.admins, user.id],
    }));
  };
